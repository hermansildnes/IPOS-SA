...
def add_products(session: Session):
    ...
    product_map = {}
    for code, name, pkg_type, unit, units_per_pack, cost, stock, min_stock in products_data:
        p = Product(
            product_code=code,
            name=name,
...
def _make_order(
    ...
    subtotal = sum(
        Decimal(str(unit_cost)) * qty for _, qty, unit_cost in items_data
    )
    ...
    order = Order(
        ...
        updated_at=datetime(delivery_date.year, delivery_date.month, delivery_date.day, tzinfo=timezone.utc),
    )
    session.add(order)
    session.flush()
...
def add_orders(session: Session, merchants: dict, products: dict, users: dict):
    ...
    print(f"  Order 1  CityPharmacy   20 Feb — subtotal £{o1.total}  due £{o1.amount_due}")
    ...
    print(f"  Order 2  Cosymed        25 Feb — subtotal £{o2.total}  due £{o2.amount_due}")
    ...
    print(f"  Order 3  HelloPharmacy  25 Feb — subtotal £{o3.total}  due £{o3.amount_due}")
    ...
    print(f"  Order 4  Cosymed        10 Mar — subtotal £{o4.total}  due £{o4.amount_due}")
    ...
    print(f"  Order 5  HelloPharmacy  25 Mar — subtotal £{o5.total}  due £{o5.amount_due}")
    ...
    print(f"  Order 6  HelloPharmacy   1 Apr — subtotal £{o6.total}  due £{o6.amount_due}")
...
def main():
    ...
    print("  HelloPharmacy  → SUSPENDED  (16 days past Mar deadline, auto-set on first read)")
    print()
